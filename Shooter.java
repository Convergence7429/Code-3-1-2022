...
public class Shooter {
    ...
    public double dumpShotSpeed = 0.4125;
    ...
    public boolean centerRobotOnTarget(double degreesOfFreedom, double speed) {
        if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -degreesOfFreedom) {
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                        -speed, true);
            } else {
                Robot.drive.driveTrainByControls(0.0, 0.0, -speed, true);
            }
            return false;
        } else if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx")
                .getDouble(0.0) > degreesOfFreedom) {
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                        speed,
                        true);
    ...
    public void dumpShot() { // 92 inches according to distance method
        ...
        if (dumpShot && !shooting) {
            ...
            if ((masterShooterMotor.getSelectedSensorVelocity() > (dumpShotSpeed * 22068.97)) && setHoodYet) {
                Robot.intake.indexerMotor.set(-0.4);
            } else {
                Robot.intake.indexerMotor.set(0.0);
            }
